...
const DeleteFunc=async (userId)=>{
  ...
  return (
    <div className="">
      {/* <div>Search</div> */}
      <ToastContainer/>
      <h1 className="text-3xl font-montserrat text-center font-bold py-8">Danh Sách Người Dùng Trong Hệ Thống</h1>
      <div className="flex flex-row">
      <p className="my-auto pr-3 text-base font-semibold text-center">Bộ lọc: </p>
      <ComboBox setRoleName={setRoleName} ></ComboBox>
      </div>
      <table className=" w-full h-96 font-montserrat text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 overflow-auto">
                ...
                <tbody className="overflow-auto">
                    {listUser && listUser.length >0 ? (listUser.slice(currentPage*10, currentPage*10+10).map((item,index)=>(
                        <tr key={index} className= {classNames(index%2!==0?'bg-blue-100':'',"font-montserrat border-b dark:bg-gray-800 dark:border-gray-700 text-xs")}>
